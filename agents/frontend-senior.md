...
- **Reach for a different motion library when the job isn't scroll storytelling.** All are verified skills with their own driver, same as `gsap`: `animejs` (`skills/animejs/SKILL.md`) for imperative, interaction-triggered sequences (click/hover micro-interactions, staggered entrances, SVG drawing) without GSAP's ScrollTrigger overhead; `react-spring` (`skills/react-spring/SKILL.md`) for physics-based, interruptible hover/press/drag feedback; `motion` (`skills/motion/SKILL.md`, npm package `motion`, the Framer Motion successor) for declarative, prop-driven animation on `motion.div` (`whileInView`, `whileHover`, `AnimatePresence`) when the animation state should live alongside component state/props. Don't mix more than one of these four in the same project without a reason — pick the one that matches the interaction, not whichever is most familiar.
